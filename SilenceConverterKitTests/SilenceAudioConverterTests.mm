...
#import <SilenceConverterKit/SilenceConverterKit.h>

@interface SilenceConverterTests : XCTestCase

@end

@implementation SilenceConverterTests
...
static void convert(void){
    ...
    OSStatus status;
    status = SilenceConverterNew(srcfmt.streamDescription, dstfmt.streamDescription, &ref);
    assert(status == noErr);
    ...
    while (remain > 0){
        status = SilenceConverterFillComplexBuffer(ref, encoderDataProc, NULL, &ioOutputDataPacketSize, abl, NULL);
        assert(status == noErr);
